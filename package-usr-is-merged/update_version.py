...
def extract_new_version(report):
    match = re.search(r"Newest version of usr-is-merged on remote site is ([\d\.]+)", report)
    if match:
...
def read_current_version():
    with open(PREPARE_SOURCE) as f:
        for line in f:
            if line.startswith("git_src"):
                match_ver=re.search(r'v(\d+)',line)
                if match_ver:
